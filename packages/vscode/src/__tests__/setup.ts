...
vi.mock('vscode', () => ({
  ...
  Uri: { file: (path: string) => ({ fsPath: path, path }), parse: (str: string) => ({ toString: () => str }) },
...
vi.mock('ira-review', () => ({
  ...
  generateTestCases: vi.fn().mockResolvedValue({ testCases: [], parseWarning: undefined }),
  annotateDiffWithLineNumbers: vi.fn((d: string) => d),
