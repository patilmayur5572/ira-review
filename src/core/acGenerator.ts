...
export function formatACsForJiraComment(
  ...
  lines.push(`💡 Suggested Acceptance Criteria from code analysis`);
  lines.push(``);
...
export function formatDiscoveredScenariosForJiraComment(
  scenarios: string[],
): string {
  const lines: string[] = [];

  lines.push(`💡 Additional scenarios spotted during implementation`);
  lines.push(``);
  lines.push(`While implementing the existing ACs, the code also ended up`);
  lines.push(`covering a few extra scenarios. Flagging them in case they're`);
  lines.push(`worth adding as formal ACs or follow-up tickets:`);
  lines.push(``);
  ...
  lines.push(``);
  lines.push(`These may already be implied by existing requirements. Just`);
  lines.push(`making them visible. Feel free to ignore if already accounted for.`);
  lines.push(``);
  lines.push(`Generated by IRA on behalf of the developer`);
